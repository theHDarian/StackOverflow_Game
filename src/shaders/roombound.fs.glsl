...
{
	if(tile){
		color=vec4(fcolor,1.f)*texture(sampler0,vec3(texcoord.x*tiling.x,texcoord.y*tiling.y,frame));
		if(texcoord.x<.1)color-=vec4(vec3(.7*(.1-texcoord.x)),0.);
		if(texcoord.x>.9)color-=vec4(vec3(.7*(texcoord.x-.9)),0.);
	}
	else{
		color=vec4(fcolor,1.f)*texture(sampler0,vec3(texcoord.x,texcoord.y,frame));
	}
