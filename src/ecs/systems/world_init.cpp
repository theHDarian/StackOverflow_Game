...
Entity createDeskWithMouse(RenderSystem *renderer, vec2 pos) {
	Entity entity = createProp3D(renderer, pos, "Mouse1.png", vec2(240 / 1.5f, 192 / 1.5f), vec2(130 / 1.5f, 0), 0,
	                             EFFECT_ASSET_ID::TEXTURED);
...
Entity createOracleTurret(RenderSystem *renderer, vec2 pos) {
	Entity entity = createProp3D(renderer, pos, "Turret.png", vec2(192, 264), vec2(110, 35), 35,
	                             EFFECT_ASSET_ID::TEXTURED);
...
Entity CreateOven(RenderSystem *renderer, vec2 pos) {
	Entity entity = createProp3D(renderer, pos, "oven.png", vec2(200, 200), vec2(100, 60), 60,
	                             EFFECT_ASSET_ID::TEXTURED);
...
Entity createPushConsole(RenderSystem *renderer, vec2 pos, std::vector<BulletStackEffect> effects) {
	...
	createWall(renderer, vec2(pos.x - 100 + c.radius * 2, pos.y + 20 - c.radius * 2),
	           vec2(pos.x + 100 - c.radius * 2, pos.y + 20 - c.radius * 2));
...
Entity createBibleTree(RenderSystem *renderer, vec2 pos) {
	...
	CircleCollider &c = registry.circleColliders.get(registry.players.entities[0]);
	createWall(renderer, vec2(pos.x - 13 + c.radius * 2, pos.y + 190 - c.radius * 2),
	           vec2(pos.x + 45 - c.radius * 2, pos.y + 190 - c.radius * 2));
...
Entity createProp(RenderSystem *renderer, vec2 pos, std::string filename, vec2 scale, vec2 shrink, bool solid) {
	...
	scale *= shrink;
	if (solid) {
		Entity ew = createWall(renderer, pos + vec2(+scale.x / 2.f, +scale.y / 2.f),
		                       pos + vec2(+scale.x / 2.f, -scale.y / 2.f));
...
Entity createProp3D(RenderSystem *renderer, vec2 pos, std::string filename, vec2 scale, vec2 wallOffset,
                    float baseOffset, EFFECT_ASSET_ID effect) {
	Entity e = Entity();
...
Entity createDoor(RenderSystem *renderer, vec2 startPos, vec2 endPos) {
	...
	door.endPos = endPos;
	door.side = (door.startPos.y == door.endPos.y)
		            ? (door.startPos.y < ws.height / 2.f)
			              ? 'B'
			              : 'T'
		            : (door.startPos.x < ws.width / 2.f)
			              ? 'L'
			              : 'R';
...
Entity createDoorSymbol(RenderSystem *renderer, char side, float angle, vec2 scale, float symbolAngle, vec3 axis,
                        vec3 offset, vec2 spriteOffset) {
	auto entity = Entity();
	Motion &motion = registry.motions.emplace(entity);
...
Entity createDoors(RenderSystem *renderer, vec2 position, float angle, vec2 scale, float doorAngle, vec3 axis,
                   vec3 offset, char side) {
	auto entity = Entity();
	Motion &motion = registry.motions.emplace(entity);
...
void createRoomBounds(RenderSystem *renderer, vec2 roomCenter, vec2 roomSize) {
	...
	std::vector<WallPos> wallPositions = {
		...
		{
			// right
			vec2(floorPosition.x + floorScale.x / 2.f - wallThickness * 1.1 / 2 - 25,
			     floorPosition.y - floorScale.y * 2 / 2.f),
			vec2(floorPosition.x + floorScale.x / 2.f - wallThickness * 1.1 / 2 - 25,
			     floorPosition.y + floorScale.y * 2 / 2.f),
		...
		{
			// bottom
			vec2(floorPosition.x - floorScale.x * 1.1 / 2.f,
			     floorPosition.y + floorScale.y / 2.f - wallThickness / 2.f),
			vec2(floorPosition.x + floorScale.x * 1.1 / 2.f,
			     floorPosition.y + floorScale.y / 2.f - wallThickness / 2.f),
			vec2(floorPosition.x, floorPosition.y - floorScale.y),
			vec2(floorScale.x * 1.1, wallThickness),
	...
	for (auto &p: wallPositions) {
		...
		b.axis = vec3(1, 0, 0);
		b.offset = p.offset;
		b.side = (p.colliderStart.y == p.colliderEnd.y)
			         ? ((p.colliderStart.y < ws.height / 2.f) ? 'B' : 'T')
			         : (p.colliderStart.x < ws.width / 2.f)
				           ? 'L'
				           : 'R';
	...
	for (auto &p: wallPositions) {
		...
		char side = (p.colliderStart.y == p.colliderEnd.y)
			            ? ((p.colliderStart.y < ws.height / 2.f) ? 'B' : 'T')
			            : (p.colliderStart.x < ws.width / 2.f)
				              ? 'L'
				              : 'R';
	...
	createDoor(renderer, {ws.width / 2 - doorwidth / 2, wallPositions[0].colliderStart.y + 45}, {
		           ws.width / 2 + doorwidth / 2, wallPositions[0].colliderStart.y + 45
	           });
	createDoor(renderer, {wallPositions[1].colliderStart.x - 30, ws.height / 2 - doorwidth / 2}, {
		           wallPositions[1].colliderStart.x - 30, ws.height / 2 + doorwidth / 2
	           });
	createDoor(renderer, {ws.width / 2 - doorwidth / 2, wallPositions[2].colliderStart.y - 30}, {
		           ws.width / 2 + doorwidth / 2, wallPositions[2].colliderStart.y - 30
	           });
	createDoor(renderer, {wallPositions[3].colliderStart.x + 30, ws.height / 2 - doorwidth / 2}, {
		           wallPositions[3].colliderStart.x + 30, ws.height / 2 + doorwidth / 2
	           });
...
Entity createTestPoly(RenderSystem *renderer, vec2 position, std::vector<vec2> points, float angle) {
	...
	for (int i = 0; i < points.size(); i++) {
		vec2 thisRotatedPoint = {
			points[i].x * cos(angle) - points[i].y * sin(angle), points[i].x * sin(angle) + points[i].y * cos(angle)
		};
		if (i == points.size() - 1) {
			vec2 nextRotatedPoint = vec2(points[0].x * cos(angle) - points[0].y * sin(angle),
			...
			vec2 nextRotatedPoint = vec2(points[i + 1].x * cos(angle) - points[i + 1].y * sin(angle),
			                             points[i + 1].x * sin(angle) + points[i + 1].y * cos(angle));
			auto line = drawLineAtoB(renderer, thisRotatedPoint + position, nextRotatedPoint + position);
			shapes.shapes.push_back(line);
		}
	}
...
Entity createEnemy(RenderSystem *renderer, vec2 pos, EnemyType type, const Entity &summoner) {
	...
	switch (type) {
		...
		case EnemyType::EnemyHifiJellyFish: enemy = HifiJellyFish();
			break;
		case EnemyType::EnemyHifiTackShooter: enemy = HifiTackShooter();
			break;
		case EnemyType::EnemyHifiBallLauncher: enemy = HifiBallLauncher();
			break;
		case EnemyType::EnemyHifiWhip: enemy = HifiWhip();
			break;
	...
	if (registry.enemyParts.has(entity)) {
		...
		if (!registry.enemies.has(ep.parent)) {
			...
			for (Entity e: registry.enemies.entities) {
				...
				if (glm::distance(motion.position, pos) < dist && !registry.enemyParts.has(e) && !registry.bossParts.
				    has(e) && !registry.roomWideBuffers.has(e)) {
					ep.parent = e;
	...
	if ((type == EnemyType::EnemySmallCShield || type == EnemyBubbleShield) && registry.motions.has(
		    registry.enemyParts.get(entity).parent)) {
		Motion &parentMotion = registry.motions.get(registry.enemyParts.get(entity).parent);
		motion.scale.x = min(max(parentMotion.scale.x, parentMotion.scale.y) * 1.75f,
		                     min(parentMotion.scale.x, parentMotion.scale.y) + 120.f);
		motion.scale.y = type == EnemyType::EnemySmallCShield
			                 ? motion.scale.x * (1.998858f / 1.923352f)
			                 : motion.scale.x;
	}
	...
	try {
		// Adjust max health if the enemy spawns in a region after its intended region
	...
	int dmgScale = registry.elites.has(entity)
		               ? (int) map.currRegion - 1 + registry.elites.get(entity).eliteLevel
		               : (int) map.currRegion - 1;
	...
	enemy.maxHealth *= pow(1.2 + (0.05 * registry.gameStates.components[0].difficulty), (max(dmgScale, 0)));
...
Entity createPopBullet(RenderSystem *renderer, vec2 pos, vec2 velocity, vec2 veer, AttackData atkData) {
	...
	if (bullet.bulletEffects[0].type == BulletEffectType::Key) {
		...
		EmitParticle &ep = registry.emitParticles.emplace(entity, PBulletTrail, props, bullet.bulletRange,
		                                                  Random::Int(3) + 5);
...
Entity createGenericPlayerBullet(RenderSystem *renderer, vec2 position, vec2 direction, float range) {
	...
	// Setting initial values
	PlayerBullet &bullet = registry.playerBullets.emplace(entity);
	bullet.damage = 0.2f * getModifiedValue(BulletDamage, bullet.damage) / min(
		                1.f, getModifiedValue(BulletNum, 1) - 0.5f);
...
std::vector<BulletStackEffect> getBulletEffects(AttackData atkData, bool &isSpecial) {
	...
	if (atkData.positiveBulletEffects.size() > 0 && Random::Float() < positiveProb && map.currRoom.preset.
	    numSpecialBulletsToSpawn > 0) {
		isSpecial = true;
		return atkData.positiveBulletEffects;
	}
	isSpecial = false;
	if (Random::Float() < 0.2f * log((float) map.currRegion) || map.currRoom.type == Testing || map.currRoom.type ==
	    TutorialRoom2) {
...
float getModifiedBulletSpeed(float bulletRange) {
	...
	return pb.bulletSpeed + clamp(-400.f, (bulletRange > 0) ? (float) bulletRange * 120.f : (float) bulletRange * -80.f,
	                              1400.f);
}
